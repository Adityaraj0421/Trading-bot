...
- **Pre-existing test failure in Phase 8**: `tests/test_decision_engine.py::TestSafetyChecks::test_defensive_on_daily_loss` fails on `main` — Phase 8 `DecisionEngine` state machine bug, unrelated to Phase 9 pipeline. Not a regression from any recent change. Use `--ignore=tests/test_decision_engine.py` when verifying Phase 9 changes.
