"""5-year backtest for BTC/USDT, ETH/USDT, SOL/USDT.

Fetches 5 years of hourly OHLCV data via CCXT pagination,
...
which gives ~16 retrains per pair instead of 208 (weekly), keeping runtime
to ~10-15 minutes total for all three pairs.
"""
...
def fetch_5yr_ohlcv(fetcher: DataFetcher, symbol: str) -> pd.DataFrame:
    """Fetch 5 years of hourly OHLCV data using paginated CCXT calls."""
...
def main() -> None:
    ...
    print("\n" + "═" * 60)
    print(f"  5-YEAR BACKTEST  |  {', '.join(PAIRS)}")
    ...
    for pair in PAIRS:
        df = fetch_5yr_ohlcv(fetcher, pair)
        if df.empty:
