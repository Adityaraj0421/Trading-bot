...
class TestPositionManagement:
    ...
    def test_open_position_deducts_capital(self, bt):
        initial = bt.capital
        ts = datetime.now()
        bt._open_position(0, ts, 50000.0, "BUY", 0.8, self._make_strat_sig("BUY"), "trending_up")
    ...
    def test_open_position_skipped_if_no_capital(self, bt):
        ...
        ts = datetime.now()
        bt._open_position(0, ts, 50000.0, "BUY", 0.8, self._make_strat_sig("BUY"), "ranging")
    ...
    def test_short_position_opens_correctly(self, bt):
        ts = datetime.now()
        bt._open_position(0, ts, 50000.0, "SELL", 0.8, self._make_strat_sig("SELL"), "trending_down")
        assert len(bt.positions) == 1
...
class TestCheckPositions:
    def _open_long(self, bt, price=50000.0):
        ts = datetime.now()
        strat = StrategySignal("BUY", 0.8, "Test", "test", suggested_sl_pct=0.02, suggested_tp_pct=0.05)
    ...
    def test_metrics_with_equity_data(self, bt):
        ...
        bt.trades.append(
            BacktestTrade(
                ...
                entry_price=50000,
                exit_price=51000,
                entry_price_actual=50025,
                exit_price_actual=50975,
                quantity=0.004,
                pnl_gross=4.0,
                pnl_net=3.8,
                fees_paid=0.1,
